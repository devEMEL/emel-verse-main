...
const CollectionSort = ({ onSort, activeSort }) => {
  const sortOptions = [
    ...
    { id: 'created', label: 'Time Created', icon: <Clock className="w-4 h-4" /> },
    { id: 'cheapest', label: 'Cheapest', icon: <DollarSign className="w-4 h-4" /> },
    { id: 'expensive', label: 'Most Expensive', icon: <DollarSign className="w-4 h-4" /> }
  ];

  return (
    <div className="flex flex-wrap gap-2 p-4 bg-zinc-900/50 rounded-lg border border-zinc-800/50">
      <span className="text-zinc-400 flex items-center mr-2">Sort by:</span>
